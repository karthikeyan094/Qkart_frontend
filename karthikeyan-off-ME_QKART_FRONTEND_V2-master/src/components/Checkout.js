...
const Checkout = () => {
  ...
  const validateRequest = (items, addresses) => {
    ...
    let totalCost = getTotalCartValue(items);
    if(balance<totalCost){
      enqueueSnackbar("You do not have enough balance in your wallet for this purchaseYou do not have enough balance in your wallet for this purchase",{variant:"warning"});
      return false;
    }
  ...
  const performCheckout = async (token, items, addresses) => {
    ...
    if(validateRequest(items,addresses)){
      try {
        ...
        let balance = localStorage.getItem("balance");
        let totalCost = getTotalCartValue(items);
        localStorage.setItem("balance",balance-totalCost);
        history.push("/thanks");
              ...
              {/* TODO: CRIO_TASK_MODULE_CHECKOUT - Display list of addresses and corresponding "Delete" buttons, if present, of which 1 can be selected */}
               {addresses.all.length>0 ? (
                addresses.all.map((item)=>{
                  return <Box key={item._id} className={addresses.selected === item._id ? "address-item selected" : "address-item not-selected"}
                  onClick={()=>{
